function [FC, POLAR]=Transmitting_Controller(FC, POLAR, ref, meas)

Re=6378;  % Radius of earth (Km) 
long= deg2rad(-78.78139);% Longitude Ground Station (Radians)
phi=deg2rad(43.000320);  % Latitude Ground Station 
elev_MSL=.183;             % elevation
R_ECEF=[(Re+elev_MSL)*cos(long)*cos(phi),...
    (Re+elev_MSL)*sin(long)*cos(phi), (Re+elev_MSL)*sin(phi)]; % takes latitude and longitude and elevation 
                                                                % (Radians)                             
